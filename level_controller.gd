...
func _ready():
	Global.current_lvl = get_tree().current_scene.scene_file_path.to_int()
	Global.bullets_on_level = level_info[Global.current_lvl][0]
	Global.orbs_on_level = level_info[Global.current_lvl][1]
...
func next_level():
	...
	if next_level_number < 8:
		...
		var next_level_path = FILE_BEGIN + str(next_level_number) + ".tscn"
		SceneTransition.get_child(2).get_child(0).get_child(0).text = "FIN"
		SceneTransition.change_scene(next_level_path)
		Global.bullets_left = level_info[next_level_number][0]
		Global.orbs_left = level_info[next_level_number][1]

func get_level_bullets():
	return level_info[Global.current_lvl][0]

func get_level_orbs():
	return level_info[Global.current_lvl][1]
